...
class QuestionSendActivity : AppCompatActivity(), View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?){
        if (requestCode == CHOOSER_REQUEST_CODE){
            ...
            val imageHeght = image.height
            val scale = Math.min(500.toFloat() / imageWidth,500.toFloat() / imageHeght)

            val matrix = Matrix()
            matrix.postScale(scale,scale)

            val resizedImage = Bitmap.createBitmap(image,0,0,imageWidth,imageHeght,matrix,true)
    ...
    override fun onClick(v:View) {
        ...
        data["utd"] = FirebaseAuth.getInstance().currentUser!!.uid
...
private fun showChooser(){
    ...
    val filmname = System.currentTimeMillis().toString() + ".jpg"
    val values = ContentValues()
    values.put(MediaStore.Images.Media.TITLE,filmname)
